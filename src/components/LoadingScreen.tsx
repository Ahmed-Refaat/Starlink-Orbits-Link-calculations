...
// Max time to show the loading screen before force-dismissing it
...
export default function LoadingScreen({ loaded }: LoadingScreenProps) {
  ...
  // Force-dismiss after MAX_LOADING_MS even if satellites never load
  useEffect(() => {
    const t = setTimeout(() => {
      setTimedOut(true);
      setFadeOut(true);
  ...
  return (
    ...
    >
      ...
      <div className="text-center z-10">
        ...
        <div className="mt-6 text-[10px] text-white/30 tracking-widest uppercase">
          Requires WebGL · Use Chrome or Firefox with hardware acceleration enabled
        </div>
      </div>
    </div>
  );
