 ...
 * Supports switching Celestrak categories via the satelliteCategory store value.
...
export function useSatellites(): UseSatellitesReturn {
  ...
  useEffect(() => {
    ...
    async function fetchWithRetry() {
      ...
      // Use /api/tle for starlink (original cached endpoint), /api/tle-category for everything else
      const url =
        satelliteCategory === 'starlink'
          ? '/api/tle'
          : `/api/tle-category?group=${encodeURIComponent(satelliteCategory)}`;
